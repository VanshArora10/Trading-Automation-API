...
def safe_last(series):
    ...
    try:
        return float(series.dropna().iloc[-1].item())
    except Exception:
        return None
...
def fetch_for_ticker(ticker):
    ...
    for tf_name, params in TIMEFRAMES.items():
        try:
            ...
            stock_info[f"{tf_name}_Open"] = float(latest["Open"].item())
            stock_info[f"{tf_name}_High"] = float(latest["High"].item())
            stock_info[f"{tf_name}_Low"] = float(latest["Low"].item())
            stock_info[f"{tf_name}_Close"] = float(latest["Close"].item())
            stock_info[f"{tf_name}_Volume"] = int(latest["Volume"].item())
            ...
            print(f"Warning: Indicator error for {ticker} @ {tf_name}: {e}")
            continue
...
def fetch_and_build_snapshot(tickers=None):
    ...
    print(f"Snapshot saved at {OUT} (fetched {len(rows)} tickers in {(datetime.now()-start).seconds}s)")
    return df
